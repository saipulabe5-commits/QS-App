...
export const SourceCodeExportModal: React.FC<SourceCodeExportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--bg-elevated)]/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[var(--bg-elevated)] w-full max-w-6xl h-[90vh] rounded-2xl shadow-2xl border border-[var(--border-primary)] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="px-6 py-4 border-b border-[var(--border-primary)] bg-[var(--bg-elevated-hover)] flex items-center justify-between flex-shrink-0">
        ...
        {data && (
          <div className="bg-[var(--bg-elevated)] text-[var(--text-primary)] px-6 py-3 border-b border-slate-200 dark:border-[var(--border-primary)] flex flex-wrap items-center justify-between gap-3 text-xs flex-shrink-0">
            <div className="flex flex-wrap items-center gap-4 text-slate-600 dark:text-slate-300">
              ...
              <div className="flex items-center space-x-1.5">
                <span className="text-[var(--text-secondary)]">Total Baris:</span>
              ...
              <div className="w-1 h-3 bg-slate-700 hidden sm:block"></div>
              <div className="flex items-center space-x-1.5">
                <span className="text-[var(--text-secondary)]">Ukuran Bundle:</span>
                <span className="font-bold text-amber-300 font-mono">{data.summary.totalSizeKb} KB</span>
              </div>
              <div className="w-1 h-3 bg-slate-700 hidden sm:block"></div>
              <div className="flex items-center space-x-1.5">
                <span className="text-[var(--text-secondary)]">Tech Stack:</span>
                <span className="text-blue-300 font-medium">React + TS + Tailwind + Vite + Express + Gemini</span>
              </div>
            </div>

            {/* View Mode Tabs */}
            <div className="flex items-center bg-[var(--bg-elevated-hover)] p-0.5 rounded-lg border border-[var(--border-primary)]">
              <button
        ...
        <div className="flex-1 flex overflow-hidden">
          {loading ? (
            <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-3">
              ...
              <p className="text-sm font-semibold text-[var(--text-primary)]">Memindai dan mengemas seluruh source code...</p>
              <p className="text-xs text-[var(--text-secondary)]">Membaca berkas TypeScript, React components, CSS, dan server logic</p>
            </div>
          ) : error ? (
            <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-3">
            ...
            <>
              ...
              <div className="w-80 border-r border-[var(--border-primary)] flex flex-col bg-[var(--bg-elevated-hover)] flex-shrink-0">
                {/* Filter and Search */}
                <div className="p-3 border-b border-[var(--border-primary)] space-y-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 text-xs bg-[var(--bg-elevated)] border border-[var(--border-primary)] rounded-lg focus:ring-2 focus:ring-blue-500/20"
                    />
                  </div>
                  ...
                  <>
                    <div className="px-4 py-2.5 bg-[var(--bg-elevated)] border-b border-slate-200 dark:border-[var(--border-primary)] flex items-center justify-between flex-shrink-0">
                      <div className="flex items-center space-x-2 truncate">
                        ...
                        <span className="px-2 py-0.5 text-[10px] bg-[var(--bg-elevated-hover)] text-[var(--text-secondary)] rounded-md font-mono">
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-[var(--bg-elevated-hover)] hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-lg transition-colors flex items-center space-x-1.5"
                      >
                    ...
                    <div className="flex-1 overflow-auto p-4 font-mono text-xs text-slate-600 dark:text-slate-300 leading-relaxed select-text">
        ...
        <div className="px-6 py-4 border-t border-[var(--border-primary)] bg-[var(--bg-elevated-hover)] flex flex-col sm:flex-row items-center justify-between gap-3 flex-shrink-0">
          ...
          <div className="flex items-center space-x-3 w-full sm:w-auto justify-end">
            <button
              onClick={handleCopyJSON}
              disabled={!data}
              className="px-4 py-2.5 bg-[var(--bg-elevated)] hover:bg-[var(--bg-elevated-hover)] text-[var(--text-primary)] border border-[var(--border-primary)] text-xs font-semibold rounded-xl transition-colors flex items-center space-x-1.5"
            >
