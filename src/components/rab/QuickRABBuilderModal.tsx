...
export const QuickRABBuilderModal: React.FC<QuickRABBuilderModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 overflow-y-auto animate-in fade-in duration-200">
      {/* Backdrop */}
      <div className="fixed inset-0 bg-[var(--bg-elevated)]/40 backdrop-blur-sm" onClick={onClose} />
      
      {/* Modal Window */}
      <div className="relative w-full max-w-4xl rounded-[20px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] overflow-hidden z-10 flex flex-col max-h-[92vh] border border-white/40 bg-[var(--bg-elevated)]/70 backdrop-blur-2xl">
        {/* HEADER MAC OS STYLE */}
        ...
        <div className="bg-[var(--bg-elevated-hover)] border-b border-[var(--border-primary)] px-6 py-3">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-[var(--bg-elevated)]/60">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-xs border border-[var(--border-primary)] rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-hidden"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-2 text-xs text-[var(--text-secondary)] font-semibold">Rp</span>
                    <input
                      type="number"
                      min={0}
                      value={projectData.targetBudget}
                      onChange={(e) => setProjectData({ ...projectData, targetBudget: Math.max(0, Number(e.target.value)) })}
                      className="w-full pl-9 pr-3 py-2 text-xs border border-[var(--border-primary)] rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-hidden font-semibold text-[var(--text-primary)]"
                    />
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 sm:col-span-2 bg-[var(--bg-elevated-hover)] p-3 rounded-xl border border-[var(--border-primary)]">
                  <div>
            ...
            <div className="space-y-4">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--bg-elevated-hover)] text-[var(--text-primary)]">
                          {opt.badge}
                        </span>
                      </div>
              ...
              {method === 'building_type' && (
                <div className="p-4 bg-[var(--bg-elevated-hover)] rounded-xl border border-[var(--border-primary)] mt-4">
                  <h4 className="text-xs font-bold text-[var(--text-primary)] mb-2">Pilih Model Tipe Bangunan:</h4>
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <div className="p-3 bg-[var(--bg-elevated-hover)] rounded-xl border border-[var(--border-primary)] flex items-center gap-2">
                <input
                  type="text"
                  value={newCustomCatName}
                  onChange={(e) => setNewCustomCatName(e.target.value)}
                  placeholder="Tambah Kategori Kustom Baru..."
                  className="flex-1 px-3 py-1.5 text-xs border border-[var(--border-primary)] rounded-lg bg-[var(--bg-elevated)] focus:outline-hidden focus:ring-2 focus:ring-blue-500"
                />
                <button
                  type="button"
                ...
                <div className="border border-[var(--border-primary)] rounded-xl overflow-hidden">
                  <div className="max-h-72 overflow-y-auto custom-scrollbar">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead className="sticky top-0 bg-[var(--bg-elevated-hover)] text-[var(--text-primary)] font-bold border-b border-[var(--border-primary)]">
                        <tr>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {draftItems.map((item) => {
                          ...
                          return (
                            <tr
                              key={item.id}
                              className={`hover:bg-[var(--bg-elevated-hover)] ${
                                !isSelected ? 'opacity-40 bg-[var(--bg-elevated-hover)]' : ''
                              }`}
                            >
                              ...
                              <td className="p-2 font-mono font-semibold text-[var(--text-primary)]">
                                <input
                                  type="text"
                                  value={item.code}
                                  onChange={(e) => updateDraftItem(item.id, 'code', e.target.value)}
                                  className="w-14 px-1 py-0.5 border border-[var(--border-primary)] rounded-sm text-[11px]"
                                />
                              </td>
                              <td className="p-2">
                                <input
                              ...
                              <td className="p-2 text-[var(--text-secondary)] text-[11px] truncate max-w-[120px]">
                                {item.category}
                              </td>
                              <td className="p-2 text-right">
                                <input
                                  ...
                                  className="w-16 px-1 py-0.5 border border-[var(--border-primary)] rounded-sm text-right text-xs"
                                />
                              </td>
                              <td className="p-2 text-center text-[var(--text-secondary)] text-[11px]">
                                {item.unit}
                              </td>
                              <td className="p-2 text-right">
                                <input
                                  ...
                                  className="w-24 px-1 py-0.5 border border-[var(--border-primary)] rounded-sm text-right text-xs font-semibold"
                                />
                              </td>
          ...
          {step === 5 && (
            <div className="space-y-4">
              ...
              {summary.targetBudget > 0 && (
                <div className="p-4 rounded-xl border border-[var(--border-primary)] bg-[var(--bg-elevated-hover)] space-y-2">
                  <div className="flex items-center justify-between text-xs">
        ...
        <div className="bg-[var(--bg-elevated-hover)] border-t border-[var(--border-primary)] px-6 py-3 flex items-center justify-between">
          <div>
            {step > 1 ? (
              <button
                type="button"
                onClick={handleBack}
                className="px-4 py-2 text-xs font-semibold text-[var(--text-primary)] bg-[var(--bg-elevated)] border border-[var(--border-primary)] rounded-xl hover:bg-[var(--bg-elevated-hover)] flex items-center gap-1.5 transition-colors"
              >
