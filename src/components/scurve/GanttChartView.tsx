...
const CATEGORY_COLORS: Record<string, { bar: string; fill: string; header: string; text: string; badge: string }> = {
  ...
  'Pekerjaan Lantai':           { bar: 'bg-teal-500',    fill: '#14b8a6', header: 'bg-teal-50',     text: 'text-teal-800', badge: 'bg-teal-100 text-teal-800 border-teal-300' },
  ...
  'Lain-lain':                  { bar: 'bg-gray-500',    fill: '#6b7280', header: 'bg-gray-50',     text: 'text-gray-700', badge: 'bg-gray-100 text-gray-700 border-gray-300' },
};
        ...
        <div className="bg-[var(--bg-elevated)] rounded-2xl p-12 border border-[var(--border-primary)] text-center space-y-4 max-w-xl mx-auto shadow-2xs">
          ...
          <div className="flex items-center justify-center gap-3">
            <button
              ...
              className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-2xs transition-colors inline-flex items-center space-x-2"
            >
      ...
      <div className="bg-[var(--bg-elevated)] text-[var(--text-primary)] rounded-2xl p-5 border border-slate-200 dark:border-[var(--border-primary)] flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <div className="flex items-center flex-wrap gap-x-3 gap-y-1 mt-2 text-xs text-[var(--text-secondary)]">
            ...
            <span>•</span>
            <span>{scurve.scheduleItems.length} Pos Pekerjaan</span>
            <span>•</span>
            <span className="text-blue-400 font-semibold">{formatRupiah(scurve.totalBudget)}</span>
          </div>
        </div>
        ...
        <div className="flex items-center flex-wrap gap-2">
          ...
          <button
            onClick={() => setActiveTab('scurve-plan')}
            className="px-3.5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl transition-colors flex items-center space-x-1.5 shadow-2xs"
          >
            ...
            {latestActualRecord ? (
              latestActualRecord.deviation >= 0 ? (
                <>
                  ...
                  <span className="text-emerald-700">+{latestActualRecord.deviation.toFixed(2)}% (Lebih Cepat)</span>
                </>
              ) : (
                <>
                  <span className="w-2.5 h-2.5 rounded-full bg-rose-500 shrink-0" />
                  <span className="text-rose-700">{latestActualRecord.deviation.toFixed(2)}% (Terlambat)</span>
                </>
              )
            ) : (
              <>
