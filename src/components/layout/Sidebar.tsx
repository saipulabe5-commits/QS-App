...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`no-print fixed top-0 bottom-0 left-0 z-40 w-72 bg-slate-50 dark:bg-[#0d0d0f] border-r border-slate-200 dark:border-slate-800 text-[var(--text-primary)] flex flex-col transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static shadow-[4px_0_24px_rgba(0,0,0,0.1)] ${ isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full' }`}
      >
        {/* Header / Brand */}
        <div className="p-5 border-b border-slate-200 dark:border-slate-500/30 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 rounded-sm bg-blue-50 dark:bg-blue-500/15 text-blue-800 border border-blue-200 dark:border-blue-500/30 dark:text-blue-300">
                  SNI
                </span>
              </div>
          ...
          {selectedProject ? (
            <div>
              ...
              <div className="flex items-center justify-between mt-1.5 text-[11px] text-[var(--text-secondary)]">
                ...
                <span
                  className={`px-1.5 py-0.5 rounded-sm font-medium ${ selectedProject.status === 'Berjalan' ? 'bg-emerald-50 dark:bg-emerald-500/15 text-emerald-700 border border-emerald-200 dark:border-emerald-500/30 dark:text-emerald-300' : selectedProject.status === 'Selesai' ? 'bg-blue-50 dark:bg-blue-500/15 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-500/30' : 'bg-slate-100 dark:bg-slate-500/15 text-slate-700 dark:text-slate-300' }`}
                >
        ...
        <div className="px-3 mb-1.5 flex-shrink-0 space-y-1.5">
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 rounded-xl bg-blue-50 dark:bg-blue-500/15 hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-800 dark:text-blue-300 border-blue-200 dark:border-blue-500/30 transition-colors group"
          >
            <div className="flex items-center space-x-2.5">
              <div className="w-6 h-6 rounded-md bg-blue-100 dark:bg-blue-500/15 text-blue-700 dark:text-blue-300 flex items-center justify-center group-hover:scale-105 transition-transform">
          ...
          {onOpenQuickBuilder && (
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2 rounded-xl bg-indigo-50 dark:bg-indigo-500/15 hover:bg-indigo-100 dark:hover:bg-indigo-900 text-indigo-800 dark:text-indigo-300 border-indigo-200 dark:border-indigo-500/30 transition-colors group"
            >
              <div className="flex items-center space-x-2.5">
        ...
        <nav className="flex-1 px-3 py-1 space-y-4 overflow-y-auto custom-scrollbar">
          {navigationSections.map((section, sIdx) => (
            <div key={sIdx}>
              <div className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.15em] px-3 mb-2 mt-2">
                {section.title}
              </div>
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${ isActive ? 'bg-blue-600' : 'text-slate-600 dark:text-slate-300 bg-white dark:bg-[#1a1a1f] hover:bg-slate-100 dark:hover:bg-[#25252c] border border-slate-200 dark:border-slate-800/50 shadow-sm hover:text-blue-600 dark:hover:text-blue-400' }`}
                    >
        ...
        <div className="p-3 m-3 bg-[var(--bg-elevated)] border-[var(--border-primary)] shadow-sm rounded-xl border border-[var(--border-primary)] flex items-center justify-between flex-shrink-0">
          ...
          {user ? (
            ...
            <button
              ...
              className="text-xs text-blue-700 dark:text-blue-300 hover:text-blue-800 dark:hover:text-blue-300 font-semibold px-2 py-1 bg-blue-50 dark:bg-blue-500/15 rounded-md border border-blue-200 dark:border-blue-500/30"
            >
              Login
            </button>
          )}
