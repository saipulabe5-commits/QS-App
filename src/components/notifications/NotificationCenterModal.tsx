...
export const NotificationCenterModal: React.FC<NotificationCenterModalProps> = ({
  ...
  const getSeverityBadge = (severity: NotificationSeverity) => {
    switch (severity) {
      case 'critical':
        return (
          <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-rose-100 dark:bg-rose-500/15 text-rose-800 dark:text-rose-300 border border-rose-200 dark:border-rose-500/30 flex items-center gap-1">
      ...
      case 'warning':
        return (
          <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-amber-100 dark:bg-amber-500/15 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-500/30 flex items-center gap-1">
      ...
      default:
        return (
          <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-blue-100 dark:bg-blue-500/15 text-blue-800 dark:text-blue-300 border border-blue-200 dark:border-blue-500/30 flex items-center gap-1">
        ...
        <div className="bg-[var(--bg-elevated-hover)] border-b border-[var(--border-primary)] px-6 py-2.5 flex items-center justify-between overflow-x-auto">
          <div className="flex items-center space-x-1">
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors ${ activeFilter === t.id ? 'bg-blue-600' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-slate-200 dark:bg-slate-500/15' }`}
              >
                {t.label}
              </button>
            ))}
          ...
          {activeFilter !== 'settings' && notifications.some((n) => !n.isRead) && (
            <button
              ...
              className="text-xs font-semibold text-blue-600 hover:text-blue-800 dark:text-blue-300 flex items-center gap-1 ml-2 whitespace-nowrap"
            >
            ...
            <div className="space-y-3">
              {filteredNotifications.length === 0 ? (
                ...
                filteredNotifications.map((notif) => (
                  <div
                    key={notif.id}
                    className={`p-4 rounded-xl border transition-all ${ notif.isRead ? 'bg-[var(--bg-elevated)] border-[var(--border-primary)] opacity-80' : 'bg-blue-50/40 dark:bg-blue-500/15 border-blue-200 dark:border-blue-500/30 shadow-2xs' }`}
                  >
                      ...
                      <div className="flex items-center space-x-1 flex-shrink-0">
                        {notif.actionLabel && notif.actionTarget && (
                          <button
                            ...
                            className="px-2.5 py-1 text-xs font-semibold text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-500/15 hover:bg-blue-100 dark:hover:bg-blue-900 border border-blue-200 dark:border-blue-500/30 rounded-lg flex items-center gap-1 transition-colors"
                          >
