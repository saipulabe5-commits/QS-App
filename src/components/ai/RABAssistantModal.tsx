...
interface RABAssistantModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialTab?: 'chat' | 'missing' | 'audit' | 'volume' | 'savings' | 'summary' | 'estimate' | 'escalation' | 'batch-pdf';
}
...
export const RABAssistantModal: React.FC<RABAssistantModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'chat' | 'missing' | 'audit' | 'volume' | 'savings' | 'summary' | 'estimate' | 'escalation' | 'batch-pdf'
  >(initialTab);
...
*Catatan: Semua saran AI tidak akan mengubah dokumen RAB Anda tanpa persetujuan Anda terlebih dahulu.*`,
  ...
  // 9. Batch PDF Extraction State
  const [batchFiles, setBatchFiles] = useState<File[]>([]);
  const [batchTaskId, setBatchTaskId] = useState<string | null>(null);
  const [pollingProgress, setPollingProgress] = useState<string | number>('0%');
  const [isPolling, setIsPolling] = useState<boolean>(false);
  const [extractedItems, setExtractedItems] = useState<ExtractedRABItem[]>([]);
  const [batchExtractionSummary, setBatchExtractionSummary] = useState<string | null>(null);
  const [batchConfidenceScore, setBatchConfidenceScore] = useState<number | null>(null);
  ...
  // Convert multiple or single files to base64
  const processFilesToBase64 = async (files: File[]): Promise<{ base64Data: string; mimeType: string }> => {
    if (files.length === 0) {
      throw new Error('Pilih minimal satu file PDF atau gambar kerja.');
    }

    // Check if there is a PDF file among selections
    ...
    if (pdfFile) {
      ...
      return { base64Data: base64, mimeType: 'application/pdf' };
    }
    ...
    if (files.length === 1) {
      ...
      return { base64Data: base64, mimeType: files[0].type || 'image/png' };
    }

    // Multiple image files: combine them vertically onto an off-screen canvas
    try {
      ...
      const maxWidth = Math.max(...imageElements.map((img) => img.naturalWidth || 1200));
      const totalHeight = imageElements.reduce((sum, img) => sum + (img.naturalHeight || 800), 0);

      const canvas = document.createElement('canvas');
      canvas.width = Math.min(maxWidth, 2400);
      const scale = canvas.width / maxWidth;
      canvas.height = Math.min(totalHeight * scale, 8000);
      ...
      if (ctx) {
        ...
        for (const img of imageElements) {
          ...
          ctx.drawImage(img, 0, currentY, canvas.width, h);
          currentY += h;
        }
        return {
      ...
      console.warn('Canvas merge failed, falling back to first file:', e);
    }

    // Fallback: single file read
    ...
    return { base64Data: fallbackBase64, mimeType: files[0].type || 'image/png' };
  };

  // Submit & Polling for Batch PDF
  const handleStartBatchPdfExtraction = async () => {
    if (batchFiles.length === 0) {
      showToast('Peringatan', 'Silakan pilih atau seret dokumen gambar kerja / PDF terlebih dahulu.', 'warning');
      return;
    }
    ...
    setIsPolling(true);
    setPollingProgress('Mengonversi dokumen & gambar kerja...');
    setExtractedItems([]);
    setBatchExtractionSummary(null);
    setBatchConfidenceScore(null);
    setDetectedDrawings([]);

    try {
      const { base64Data } = await processFilesToBase64(batchFiles);
      setPollingProgress('Mendaftarkan tugas ke antrean asinkron server...');
      ...
      setPollingProgress('Tugas masuk antrean. Memindai lembar gambar kerja... (25%)');
      ...
      pollingIntervalRef.current = setInterval(async () => {
        try {
          ...
          if (statusRes.status === 'processing') {
            ...
            let label = `Menganalisis lembar gambar kerja... (${prog}%)`;
            if (prog <= 30) {
              label = `Memindai notasi & legenda gambar kerja... (${prog}%)`;
            } else if (prog <= 60) {
              label = `Mengekstrak dimensi & spesifikasi teknis (Zero Hallucination)... (${prog}%)`;
            } else if (prog <= 85) {
              label = `Memetakan item ke standar AHSP & kalkulasi volume... (${prog}%)`;
            }
            setPollingProgress(label);
            ...
            setIsPolling(false);
            setPollingProgress('Ekstraksi selesai 100%');

            const res = statusRes.result;
            if (res && Array.isArray(res.rabItems) && res.rabItems.length > 0) {
              setExtractedItems(res.rabItems);
              setBatchExtractionSummary(res.summary || 'Ekstraksi gambar kerja berhasil dipetakan ke item RAB.');
              setBatchConfidenceScore(res.confidenceScore ?? 92);
              setDetectedDrawings(res.detectedDrawings || []);
              showToast('Ekstraksi Berhasil', `Berhasil mengekstrak ${res.rabItems.length} item pekerjaan teknis.`, 'success');
            } else {
              setExtractedItems([]);
              setBatchExtractionSummary(res?.summary || 'Dokumen diproses namun tidak ditemukan tabel/notasi pekerjaan teknis.');
              showToast('Ekstraksi Selesai', 'Tidak ditemukan item pekerjaan yang valid dari dokumen ini.', 'info');
            }
            ...
            setIsPolling(false);
            setPollingProgress('Gagal memproses ekstraksi');
            showToast('Gagal Ekstraksi', statusRes.error || 'Terjadi kesalahan saat memproses ekstraksi dokumen.', 'error');
          }
        } catch (pollErr: any) {
      ...
      setIsPolling(false);
      setPollingProgress('Gagal menyiapkan dokumen');
      showToast('Gagal Memulai', err.message || 'Gagal mengirim tugas ekstraksi PDF.', 'error');
    }
  };

  // Open Review & Approval Modal for Extracted Items
  const handleOpenReviewBatchPdf = () => {
    if (extractedItems.length === 0) return;
    setReviewModalProps({
      title: 'Tinjau & Setujui Ekstraksi PDF Gambar Kerja',
      description: 'Periksa daftar item pekerjaan yang diekstraksi AI dari dokumen gambar teknis sebelum dimasukkan ke dokumen RAB.',
      mode: 'add_items',
      proposedItems: extractedItems.map((item) => ({
        code: item.code,
        name: item.name,
        ...
        reason: item.evidence ? `Evidence: ${item.evidence}` : 'Hasil ekstraksi Agentic PDF Vision & QS',
        notes: item.evidence ? `Evidence: ${item.evidence}` : undefined,
        selected: true,
      })),
    });
    setReviewModalOpen(true);
      ...
      >
        ...
        <div className="bg-[var(--bg-elevated-hover)] px-4 py-2 flex items-center space-x-1 overflow-x-auto custom-scrollbar flex-shrink-0 border-b border-[var(--border-primary)]">
          {[
            ...
            { id: 'batch-pdf', label: 'Ekstrak PDF (Batch)', icon: Layers },
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar bg-[var(--bg-elevated-hover)]">
          ...
          {/* TAB 9: AGENTIC PDF RAB EXTRACTOR (BATCH) */}
          {activeTab === 'batch-pdf' && (
            <div className="space-y-4">
              ...
              <div className="bg-[var(--bg-elevated)] p-5 rounded-xl border border-[var(--border-primary)] shadow-2xs space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--border-primary)] pb-3">
                  <div>
                    <h4 className="text-sm font-bold text-[var(--text-primary)] flex items-center space-x-2">
                      ...
                      <span>Agentic PDF & Gambar Kerja RAB Extractor</span>
                    </h4>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                      Ekstraksi otomatis pos pekerjaan, dimensi volume, dan spesifikasi teknis dari dokumen gambar kerja dengan Asynchronous Task Queue.
                    </p>
                  </div>
                  <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold bg-blue-500/10 text-blue-600 border border-blue-500/20 shrink-0">
                    Zero-Hallucination Vision AI
                  </span>
                </div>
                ...
                <div
                  ...
                  className="border-2 border-dashed border-[var(--border-primary)] hover:border-blue-500 rounded-xl p-6 text-center cursor-pointer transition-all bg-[var(--bg-elevated-hover)]/40 hover:bg-[var(--bg-elevated-hover)] group"
                >
                  <input
                    ref={fileInputRef}
                    type="file"
                    ...
                    id="batch-pdf-input"
                  />
                  <div className="w-12 h-12 rounded-2xl bg-blue-500/10 text-blue-600 flex items-center justify-center mx-auto mb-3 group-hover:scale-105 transition-transform">
                    <UploadCloud className="w-6 h-6" />
                  </div>
                  <p className="text-xs font-bold text-[var(--text-primary)] mb-1">
                    Klik untuk memilih atau seret gambar kerja / PDF ke sini
                  </p>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                    Mendukung banyak file sekaligus: Dokumen PDF, Gambar Denah/Potongan (.PNG, .JPG)
                  </p>
                </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-semibold text-[var(--text-primary)]">
                        File Terpilih ({batchFiles.length} file)
                      </span>
                      <button
                        type="button"
                        onClick={handleClearFiles}
                        disabled={isPolling}
                        className="text-[11px] text-rose-500 hover:text-rose-600 font-medium disabled:opacity-50 cursor-pointer"
                      >
                        Hapus Semua
                      </button>
                    </div>
                    <div className="max-h-36 overflow-y-auto space-y-1.5 custom-scrollbar">
                      {batchFiles.map((file, idx) => (
                        <div
                          key={idx}
                          className="flex items-center justify-between px-3 py-2 bg-[var(--bg-elevated-hover)] rounded-lg border border-[var(--border-primary)] text-xs"
                        >
                ...
                <button
                  type="button"
                  onClick={handleStartBatchPdfExtraction}
                  disabled={isPolling || batchFiles.length === 0}
                  className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed text-white text-xs font-bold rounded-xl shadow-2xs flex items-center justify-center space-x-2 transition-colors cursor-pointer"
                >
                  {isPolling ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span>Sedang Memproses di Background Task Queue...</span>
                    </>
                  ) : (
                    <>
                      <Layers className="w-4 h-4" />
                      <span>Mulai Ekstraksi Gambar Kerja & PDF (Batch)</span>
                    </>
                  )}
                </button>
              </div>
              ...
              {isPolling && (
                <div className="bg-[var(--bg-elevated)] p-5 rounded-xl border border-blue-500/30 shadow-2xs space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2.5">
                      ...
                      <span className="text-xs font-bold text-[var(--text-primary)]">
                        Antrean Latar Belakang Aktif
                      </span>
                    </div>
                    {batchTaskId && (
                      <span className="text-[10px] font-mono bg-blue-500/10 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded-md border border-blue-500/20">
                        ID: {batchTaskId.substring(0, 8)}...
                      </span>
                    )}
                  </div>

                  <div className="p-3 bg-[var(--bg-elevated-hover)] rounded-lg border border-[var(--border-primary)] flex items-center space-x-3">
                    <Loader2 className="w-5 h-5 text-blue-600 animate-spin shrink-0" />
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-semibold text-[var(--text-primary)] truncate">
                        {String(pollingProgress)}
                      </div>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                        Dokumen diproses di background worker server tanpa risiko HTTP Timeout.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[var(--bg-elevated)] p-5 rounded-xl border border-[var(--border-primary)] shadow-2xs space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[var(--border-primary)] pb-3">
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs font-bold text-[var(--text-primary)]">
                          Hasil Ekstraksi ({extractedItems.length} Pos Pekerjaan)
                        </span>
                        {batchConfidenceScore !== null && (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">
                            {batchConfidenceScore}% Confidence
                          </span>
                        )}
                      </div>
                    ...
                    <button
                      type="button"
                      onClick={handleOpenReviewBatchPdf}
                      className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl flex items-center justify-center space-x-1.5 shadow-2xs shrink-0 cursor-pointer"
                    >
                      <ShieldCheck className="w-4 h-4" />
                      <span>Tinjau & Terapkan ke RAB</span>
                    </button>
                  </div>

                  {/* Detected drawings tags */}
                  {detectedDrawings.length > 0 && (
                    <div className="flex items-center flex-wrap gap-1.5">
                      <span className="text-[11px] font-semibold text-[var(--text-secondary)] mr-1">
                        Lembar Terdeteksi:
                      </span>
                      {detectedDrawings.map((drawing, dIdx) => (
                        <span
                          key={dIdx}
                          className="text-[10px] font-medium px-2 py-0.5 rounded-md bg-[var(--bg-elevated-hover)] text-[var(--text-primary)] border border-[var(--border-primary)]"
                        >
                          {drawing}
                        </span>
                      ))}
                  ...
                  <div className="border border-[var(--border-primary)] rounded-xl overflow-hidden max-h-72 overflow-y-auto custom-scrollbar">
                    <table className="w-full text-left text-xs text-[var(--text-secondary)]">
                      <thead className="bg-[var(--bg-elevated-hover)] text-[var(--text-primary)] font-semibold text-[10px] uppercase sticky top-0 z-10">
                        <tr>
                          <th className="px-3 py-2">Kode</th>
                          <th className="px-3 py-2">Uraian Pekerjaan & Kategori</th>
                          <th className="px-2 py-2 text-center">Sat</th>
                          <th className="px-3 py-2 text-right">Volume</th>
                          <th className="px-3 py-2 text-right">Harga (Rp)</th>
                          <th className="px-3 py-2 text-right">Subtotal (Rp)</th>
                          <th className="px-3 py-2">Bukti Gambar (Evidence)</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[var(--border-primary)]">
                        {extractedItems.map((item, idx) => (
                          <tr key={idx} className="hover:bg-[var(--bg-elevated-hover)] transition-colors">
                            ...
                            <td className="px-2 py-2 text-center font-mono text-[11px]">{item.unit}</td>
                            <td className="px-3 py-2 text-right font-mono font-medium">
                            ...
                            <td className="px-3 py-2 text-[11px] text-slate-500 dark:text-slate-400">
                              <span
                                className="inline-block max-w-xs truncate bg-[var(--bg-elevated-hover)] px-2 py-0.5 rounded text-[10px] border border-[var(--border-primary)]"
                                title={item.evidence}
                              >
                                {item.evidence || 'Terdeteksi dari notasi gambar'}
                              </span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot className="bg-[var(--bg-elevated-hover)] border-t border-[var(--border-primary)] font-bold text-[var(--text-primary)] text-xs">
                        <tr>
                          <td colSpan={5} className="px-3 py-2.5 text-right uppercase text-[10px]">
                            Total Estimasi Nilai Ekstraksi:
                          </td>
                          <td className="px-3 py-2.5 text-right font-mono text-blue-600 dark:text-blue-400">
                            {formatRupiah(
                              extractedItems.reduce((sum, it) => sum + (it.volume * it.unitPrice || 0), 0)
                            )}
                          </td>
                          <td></td>
                        </tr>
