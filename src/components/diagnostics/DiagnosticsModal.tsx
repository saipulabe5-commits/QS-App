...
export const DiagnosticsModal: React.FC<DiagnosticsModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'boot' | 'storage' | 'financial' | 'system'>('boot');
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          className="relative w-full max-w-3xl bg-slate-900 text-slate-100 rounded-2xl shadow-2xl border border-slate-800 overflow-hidden flex flex-col max-h-[85vh]"
        ...
        >
          ...
          <div className="px-5 py-4 border-b border-slate-800 bg-slate-950/80 flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">Pemeriksaan integritas boot, single source of truth, dan database</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          {/* Tab Navigation */}
          <div className="px-5 pt-3 pb-1 border-b border-slate-800 flex space-x-4 text-xs font-semibold">
            <button
          ...
          <div className="p-5 overflow-y-auto flex-1 space-y-4 custom-scrollbar text-xs">
            {activeTab === 'boot' && (
              <div className="space-y-3">
                ...
                <div className="bg-slate-950 rounded-xl border border-slate-800 p-3 space-y-2 font-mono text-[11px] max-h-72 overflow-y-auto">
                  {bootEvents.map((evt, idx) => (
                    <div key={idx} className="flex items-start space-x-2">
                      ...
                      <span className="text-slate-300 font-semibold">{evt.phase}</span>
                      {evt.detail && <span className="text-slate-400 truncate">- {evt.detail}</span>}
                    </div>
                  ))}
                </div>
              </div>
